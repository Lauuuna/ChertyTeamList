...
async function loadPlayerProfile(username) {
    try {
        const [playerData, allLevels, order] = await Promise.all([
            ...
            fetch('https://ctl.launa-fic.workers.dev/api/levels').then(res => res.json()),
            fetch('https://ctl.launa-fic.workers.dev/api/order').then(res => res.json())
        ]);
...
async function fetchPlayerData(username) {
    const [usersResponse, levelsResponse, orderResponse] = await Promise.all([
        fetch('https://cl-proxy.launa-fic.workers.dev/api/users'),
        fetch('https://cl-proxy.launa-fic.workers.dev/api/levels'),
    ...
    return `
        <div class="level-card clickable-level" data-id="${level.id}">
            <div class="level-preview">
                ...
                <img src="${imgSrc}" alt="${level.name}" loading="lazy" onerror="this.src='https://via.placeholder.com/300x180/333/666?text=No+Preview'">
            </div>
