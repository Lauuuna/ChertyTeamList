...
async function handleLogin() {
    ...
    try {
        const res = await fetch('https://ctl.launa-fic.workers.dev/api/login', {
            method: 'POST',
...
async function loadUserProfile(username) {
    try {
        const res = await fetch(`https://ctl.launa-fic.workers.dev/api/profile/${username}`, {
...
async function saveLevelData() {
    ...
    try {
        const res = await fetch('https://ctl.launa-fic.workers.dev/api/levels', {
            method: 'PUT',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${authToken}`
...
async function saveLevelOrder() {
    ...
    try {
        const res = await fetch('https://ctl.launa-fic.workers.dev/api/order', {
            method: 'PUT',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${authToken}`
...
async function loadDemonList() {
    try {
        const [orderResponse, levelsResponse] = await Promise.all([
            fetch('https://ctl.launa-fic.workers.dev/api/order'),
            fetch('https://ctl.launa-fic.workers.dev/api/levels')
        ]);
...
async function loadArchiveLevels(type = 'official') {
    try {
        const response = await fetch(`https://cl-proxy.launa-fic.workers.dev/api/${type}_archive`);
