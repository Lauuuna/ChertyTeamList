...
async function loadLevelData(levelId) {
    try {
        const response = await fetch('https://ctl.launa-fic.workers.dev/api/levels');
        ...
        const orderResponse = await fetch('https://ctl.launa-fic.workers.dev/api/order');
...
function updateLevelPage(level, position) {
    ...
    document.querySelectorAll('.info-card')[2].querySelector('.info-value').textContent = 
        level.skill_sets ? level.skill_sets.join(', ') : 'None';
